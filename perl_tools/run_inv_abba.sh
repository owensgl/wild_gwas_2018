...
working_directory="/media/owens/Copper/wild_gwas_2018/texanus" #Where all the output files are going to be produced
wild_gwas_git="/home/owens/bin/wild_gwas_2018/"
inversion_version="v3" #Examples mar27, v3.
outgroups="GRO_2043,DIV_1956"
xrq_ref="/scratch/gowens/ref/HanXRQr1.0-20151230.fa"
n_cores=10 #Number of cores for IQtree
...
mds=${direction}$mds_n
data_prefix="Texanus.tranche90.snp.gwas.90.bi.remappedHa412"
...
cat $wild_gwas_git/MDS_outliers/Ha412HO/$species/Ha412HO_inv.$inversion_version.pcasites.$chr.$mds.genotypes.txt | grep -f ../../${data_prefix}.samplelist.txt | awk '($2 != "NA") && ($6 == 2) && ($4 > 0.85)' | shuf | head -n 2 | cut -f 1 > group_2.txt
cat $wild_gwas_git/MDS_outliers/Ha412HO/$species/Ha412HO_inv.$inversion_version.pcasites.$chr.$mds.genotypes.txt |grep -f ../../${data_prefix}.samplelist.txt | awk '($2 != "NA") && ($6 == 0) && ($4 < 0.15)' | shuf | head -n 2 | cut -f 1 > group_0.txt
