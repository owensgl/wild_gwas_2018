...
env_associations <- read_tsv("/media/owens/Copper/wild_gwas/env_associations/for_Greg/var_out_petfal_2018_HA412_maf_0.03_all_covariates_BayPass_IS_STD.txt",
...
petfal_inversions_length <- petfal_inversions %>%
  mutate(length = end - start) %>%
  group_by(chr, mds) %>%
  summarize(length=round(sum(length)/1000000))

petfal_inversion_list <- petfal_inversions %>% select(chr, mds) %>% unique()
...
for (i in 1:nrow(petfal_inversions)){
  chosen_chr = petfal_inversions$chr[i]
  chosen_start = petfal_inversions$start[i]
  chosen_end = petfal_inversions$end[i]
  all_windows %>%
...
for (i in 1:nrow(petfal_inversion_list)){
  chosen_chr <- petfal_inversion_list$chr[i]
  chosen_mds <- petfal_inversion_list$mds[i]
  print(paste("Testing",chosen_chr,chosen_mds))
  env_associations_inv <- env_associations %>% filter(chr == "NANA") #Create empty tibble
  for( n in 1:nrow(petfal_inversions)){
    if (chosen_chr != petfal_inversions$chr[n]){next}
    if (chosen_mds != petfal_inversions$mds[n]){next}
    env_associations %>%
      filter(chr == chosen_chr, pos > petfal_inversions$start[n], pos < petfal_inversions$end[n]) -> tmp
...
for (x in 1:nrow(petfal_inversion_list)){
  chosen_chr <- petfal_inversion_list$chr[x]
  chosen_mds <- petfal_inversion_list$mds[x]
  chosen_windows <- petfal_inversions_length %>% filter(chr == chosen_chr, mds == chosen_mds) %>% pull(length)
...
pdf("var_out_petfal_2018_HA412_maf_0.03_all_covariates_BayPass_IS_STD.null_w.pdf")
inv_p_scores %>%
...
pdf("var_out_petfal_2018_HA412_maf_0.03_all_covariates_BayPass_IS_STD.null_w.hist.pdf")
hist(inv_p_scores$empirical_p)
dev.off()

write_tsv(perm_z_scores, "var_out_petfal_2018_HA412_maf_0.03_all_covariates_BayPass_IS_STD.null_w.perm.txt")
write_tsv(inv_z_scores, "var_out_petfal_2018_HA412_maf_0.03_all_covariates_BayPass_IS_STD.null_w.inv_z.txt")
write_tsv(inv_p_scores, "var_out_petfal_2018_HA412_maf_0.03_all_covariates_BayPass_IS_STD.null_w.txt")
