...
library(qvalue)
library(forcats)
prefix <- "Ha412HO_inv.jan09.pcasites.PET."
...
pcavalues <- read_delim("PCA/Petiolaris.tranche90.snp.petfal.90.bi.remappedHa412HO.v3noinv.ldfilter.ldr0p2.eigenvectors.txt",
                        col_names = paste("PC",1:614,sep=""),
                        delim=" ")

phenotypes <- read_tsv("gwas/petfal/PETFAL_all_phenotypes_jan_2019.txt") %>%
  rename(sample = FID)
...
inversion_gwas_popstr %>%
  ...
  mutate(sig = case_when(q < 0.05 ~ "sig",
                         TRUE ~ "nonsig")) %>%
  inner_join(.,trait_guide) %>%
  write_tsv("gwas/petfal/Ha412HO_inv.v3.allgwas.popstr.txt")
...
inversion_gwas_nostr %>%
  ...
  mutate(sig = case_when(q < 0.05 ~ "sig",
                         TRUE ~ "nonsig")) %>%
  inner_join(.,trait_guide) %>%
  write_tsv("gwas/petfal/Ha412HO_inv.v3.allgwas.nostr.txt")
...
inversion_gwas_nostr %>%
  ...
  geom_point(data=inversion_gwas_popstr %>%
               ...
               mutate(sig = case_when(q < 0.05 ~ "sig",
                                      TRUE ~ "nonsig")) %>%
               filter(sig == "sig"),
             aes(x=paste0("Chr",sprintf("%02d", chr),".",direction,mds),y=trait,size=abs(log10(q))),shape='*',color="red") +
  theme(axis.text.x = element_text(angle = 60, hjust = 1, vjust = 1),
        panel.grid.major.y = element_blank()) +
  coord_cartesian(xlim=c(0.5,n_inversions+8))+
  ylab("Trait") + xlab("Inversion") +
  ggtitle("PetFal inversion GWAS")-> p
...
pdf("gwas/Ha412HO_inv.v3.pcasites.PETFAL.gwas.v1.pdf",height=4,width=6)
p
dev.off()
...
inversion_gwas_nostr %>%
  ...
  geom_point(data = inversion_gwas_nostr %>%
             ...
             aes(x=inversion,y=trait),color="blue",shape=1,size=4) +
  scale_fill_manual(values=c("black","white")) +
  geom_text(data=inversion_gwas_popstr %>%
              inner_join(.,trait_guide) %>%
              mutate(sig = case_when(q < 0.05 ~ "sig",
                                     TRUE ~ "nonsig")) %>%
              filter(sig == "sig"),
            aes(x=paste0("Chr",sprintf("%02d", chr),".",direction,mds),y=trait,label=round(abs(log10(q)))),color="black",size=3) +
  theme(axis.text.x = element_text(angle = 60, hjust = 1, vjust = 1),
        panel.grid.major.y = element_blank()) +
  coord_cartesian(xlim=c(0.5,n_inversions+8))+
  ylab("Trait") + xlab("Inversion") +
