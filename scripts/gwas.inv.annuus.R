...
library(jtools)
library(qvalue)
prefix <- "Ha412HO_inv.v3.pcasites.ANN."
...
pcavalues <- read_delim("PCA/Annuus.tranche90.snp.gwas.90.bi.remappedHa412HO.v3noinv.ldfilter.ldr0p2.eigenvectors.txt",
                        col_names = paste("PC",1:614,sep=""),
                        delim=" ")

phenotypes <- read_tsv("gwas/ann/ANN_all_phenotypes_jan_2019.txt") %>%
  rename(sample = FID)
...
inversion_gwas_popstr %>%
  ...
  mutate(sig = case_when(q < 0.05 ~ "sig",
                         TRUE ~ "nonsig")) %>%
  inner_join(.,trait_guide) %>%
  write_tsv("gwas/ann/Ha412HO_inv.v3.allgwas.popstr.txt")
...
inversion_gwas_nostr %>%
  ...
  mutate(sig = case_when(q < 0.05 ~ "sig",
                         TRUE ~ "nonsig")) %>%
  inner_join(.,trait_guide) %>%
  write_tsv("gwas/ann/Ha412HO_inv.v3.allgwas.nostr.txt")
...
inversion_gwas_nostr %>%
  ...
  mutate(sig = case_when(q < 0.05 ~ "sig",
                         TRUE ~ "nonsig")) %>%
  mutate(trait = fct_reorder(trait, trait_types)) %>%
  mutate(trait = fct_rev(trait)) %>%
  mutate(inversion = paste0("Chr",sprintf("%02d", chr),".",direction,mds)) %>%
...
pdf("gwas/Ha412HO_inv.v3.pcasites.ANN.gwas.v1.pdf",height=4,width=6)
p
dev.off()
...
pdf("gwas/Ha412HO_inv.v3.pcasites.ANN.gwas.v2.pdf",height=4,width=6)
p
dev.off()  
