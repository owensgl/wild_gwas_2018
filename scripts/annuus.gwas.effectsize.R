...
inversion_pvalues <- read_tsv(paste("gwas/",prefix,"invgwas.txt",sep=""))
...
for (i in 1:length(traits)){
  ...
  for (n in 1:n_inv){
    ...
    info <- read_tsv(paste("MDS_outliers/Ha412HO/",chosen_noncapital_species,"/Ha412HO_inv.jan09.pcasites.",chosen_chr,".",chosen_mds,".genotypes.txt",sep="")) %>%
      ...
      mutate( triangle_genotype = as.factor(triangle_genotype)) %>%
  ...
  if (!file_test("-f", paste(directory,"/annuus/emmax/", "Annuus.tranche90.snp.gwas.90.bi.remappedHa412HO.beagle.jan09noinv.",chosen_trait,".znorm.ps.gz",sep=""))){
    next
  }
  snps <- read_tsv(paste(directory,"/annuus/emmax/", "Annuus.tranche90.snp.gwas.90.bi.remappedHa412HO.beagle.jan09noinv.",chosen_trait,".znorm.ps.gz",sep=""),
...
read_tsv(paste(directory,"/annuus/Annuus.tranche90.snp.gwas.90.bi.remappedHa412HO.beagle.jan09noinv.tped",sep=""),
...
pdf(paste("gwas/",prefix,"pve.v1.pdf"),width=12,height=8)
PVE %>% 
