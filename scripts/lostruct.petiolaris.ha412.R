...
data_name <- "Petiolaris.tranche90.snp.gwas.90.bi.remappedHa412"
...
for (i in 1:nrow(sig_mds_clusters)){
  ...
  pca_plot_normal <- out.normal %>%
    inner_join(.,labels) %>%
    ggplot(.,aes(x=PC1,y=PC2)) + geom_point(aes(color=species)) + theme_bw() +
...
out.rotation %>%
  ...
  full_join(., read_tsv("dune_sample_info.txt")) %>% View()
...
ggplot(.,aes(x=PC1,y=PC2)) + geom_point(aes(color=species),size=4) + theme_bw() +
