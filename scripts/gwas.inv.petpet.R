...
pcavalues <- read_delim("PCA/Petiolaris.tranche90.snp.petpet.90.bi.remappedHa412HO.v3noinv.ldfilter.ldr0p2.eigenvectors.txt",
                        col_names = paste("PC",1:614,sep=""),
                        delim=" ")

phenotypes <- read_tsv("gwas/petpet/PETPET_all_phenotypes_jan_2019.txt") %>%
  rename(sample = FID)
...
inversion_gwas_popstr %>%
  ...
  mutate(sig = case_when(q < 0.05 ~ "sig",
                         TRUE ~ "nonsig")) %>%
  inner_join(.,trait_guide) %>%
  write_tsv("gwas/petpet/Ha412HO_inv.v3.allgwas.popstr.txt")
...
inversion_gwas_nostr %>%
  ...
  mutate(sig = case_when(q < 0.05 ~ "sig",
                         TRUE ~ "nonsig")) %>%
  inner_join(.,trait_guide) %>%
  write_tsv("gwas/petpet/Ha412HO_inv.v3.allgwas.nostr.txt")
...
inversion_gwas_nostr %>%
  ...
  ggtitle("PetPet inversion GWAS")-> p
...
p
pdf("gwas/Ha412HO_inv.v3.pcasites.PETPET.gwas.v1.pdf",height=4,width=6)
p
dev.off()
