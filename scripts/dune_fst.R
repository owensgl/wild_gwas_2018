...
#######
directory <- "/media/owens/Copper/wild_gwas/wild_gwas/petiolaris"
...
####Both together
pdf("dune_fst_v2.pdf",height=5,width=12)
gsd.fst.cum %>%
  filter(N1 > 5 & N2 > 5) %>%
  filter(Fst > 0.05) %>%
  ggplot(.) +
  ...
  geom_line(data=gsd.fst.cum %>% 
              ...
              filter(n_sites > 10),
            aes(x=window_middle,y=meanFst),color="red",alpha=0.8) +
  geom_line(data=mon.fst.cum %>% 
              filter(N1 > 5 & N2 > 5) %>%
              mutate(window = floor(poscum/window_size),window_middle=(window*window_size)+(window_size/2)) %>%
              group_by(chr,window,window_middle) %>%
              summarize(meanFst = sum(FstNum)/sum(FstDenom),n_sites = n()) %>%
              filter(n_sites > 10),
            aes(x=window_middle,y=meanFst),color="blue",alpha=0.8) +
