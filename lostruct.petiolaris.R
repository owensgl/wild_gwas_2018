...
bcf.file <- "/media/owens/Copper/wild_gwas_2018/petiolaris/Petiolaris.tranche90.snp.gwas.90.bi.bcf"
samples <- read_tsv("/media/owens/Copper/wild_gwas_2018/petiolaris/Petiolaris.tranche90.snp.gwas.90.bi.samplelist.txt",col_names = F)
...
pdf("Petiolaris.tranche90.snp.gwas.90.bi.lostruct.1000.v0.pdf",height=16,width=25)
win.regions %>%
...
saveRDS(all_pcs, file = "Petiolaris.tranche90.snp.gwas.90.bi.lostruct.1000.all_pcs.rds")
...
pdf("Petiolaris.tranche90.snp.gwas.90.bi.lostruct.1000.closeup.v0.pdf")
...
all_pcs %>%
  ...
  ggtitle("MDS 4a") +
  facet_wrap(~mds,nrow=4)

all_pcs %>%
  filter(chrom == "HanXRQChr05") %>%
  select(chrom, start, end, mid, mds5, mds7) %>%
  distinct(chrom, start, end, mid, mds5, mds7) %>% 
  gather(., mds, value,  mds5, mds7) %>%
  ggplot(.,aes(x=mid/1000000,y=value)) + geom_point() +
  theme_bw() + xlab("MB") + ylab("MDS") +
  ggtitle("MDS 5b") +
...
all_pcs %>%
  ...
  select(chrom, start, end, mid,mds3,  mds6,mds7) %>%
  distinct(chrom, start, end, mid,mds3,  mds6,mds7) %>% 
  gather(., mds, value, mds3,  mds6,mds7) %>%
  ggplot(.,aes(x=mid/1000000,y=value)) + geom_point() +
  theme_bw() + xlab("MB") + ylab("MDS") +
  ggtitle("MDS 9a") +
...
all_pcs %>%
  ...
  select(chrom, start, end, mid,mds2, mds6 ) %>%
  distinct(chrom, start, end, mid,mds2, mds6) %>% 
  gather(., mds, value, mds2, mds6) %>%
  ggplot(.,aes(x=mid/1000000,y=value)) + geom_point() +
  theme_bw() + xlab("MB") + ylab("MDS") +
  ggtitle("MDS 12a") +
  facet_wrap(~mds,nrow=5)
...
all_pcs %>%
  ...
  select(chrom, start, end, mid,mds2, mds6,mds8 ) %>%
  distinct(chrom, start, end, mid,mds2, mds6,mds8) %>% 
  gather(., mds, value, mds2, mds6,mds8) %>%
  ggplot(.,aes(x=mid/1000000,y=value)) + geom_point() +
  theme_bw() + xlab("MB") + ylab("MDS") +
  ggtitle("MDS 14b") +
  facet_wrap(~mds,nrow=5)
...
all_pcs %>%
  filter(chrom == "HanXRQChr15") %>%
  select(chrom, start, end, mid,mds1 ) %>%
  distinct(chrom, start, end, mid,mds1) %>% 
  gather(., mds, value, mds1) %>%
  ggplot(.,aes(x=mid/1000000,y=value)) + geom_point() +
  theme_bw() + xlab("MB") + ylab("MDS") +
  ggtitle("MDS 15a") +
  facet_wrap(~mds,nrow=5)
...
all_pcs %>%
  ...
  select(chrom, start, end, mid,mds3, mds4,mds6 ) %>%
  distinct(chrom, start, end, mid,mds3, mds4,mds6) %>% 
  gather(., mds, value, mds3, mds4,mds6) %>%
  ggplot(.,aes(x=mid/1000000,y=value)) + geom_point() +
  theme_bw() + xlab("MB") + ylab("MDS") +
  ggtitle("MDS 17a") +
  facet_wrap(~mds,nrow=5)
...
inversions = c("4a", "5b", "7a","9a","10b","11b","12a","14b","15a","17a")


pdf("Petiolaris.tranche90.snp.gwas.90.bi.inversions.pdf")
all_calls <- tibble(name=character(),haplotype=numeric(),region=character())
for (inversion in inversions){
  struc = read_delim(paste("/media/owens/Copper/wild_gwas_2018/petiolaris/Petiolaris.tranche90.snp.gwas.90.bi.",inversion,".2.meanQ",sep=""),col_names = F,
                     delim=" ")
  
  het = read_tsv(paste("/media/owens/Copper/wild_gwas_2018/petiolaris/Petiolaris.tranche90.snp.gwas.90.bi.",inversion,".het.txt",sep=""))
  ...
  gds.file <- paste("Petiolaris.tranche90.snp.gwas.90.bi.",inversion,".gds",sep="")
  vcf.file <- paste("Petiolaris.tranche90.snp.gwas.90.bi.",inversion,".vcf",sep="")
...
pdf("Petiolaris.tranche90.snp.gwas.90.bi.inversions.genotyped.pdf")
...
all_calls %>% 
  inner_join(labels) %>%
  write_tsv(.,"Petiolaris.tranche90.snp.gwas.90.bi.inversions.genotyped.txt")
