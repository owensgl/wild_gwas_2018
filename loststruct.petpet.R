...
bcf.file <- "/media/owens/Copper/wild_gwas_2018/petpet/Petiolaris.tranche90.snp.petpet.90.bi.bcf"
samples <- read_tsv("/media/owens/Copper/wild_gwas_2018/petpet/Petiolaris.tranche90.snp.petpet.90.bi.samplelist.txt",col_names = F)
...
pdf("Petiolaris.tranche90.snp.petpet.90.bi.lostruct.1000.v0.pdf",height=16,width=25)
win.regions %>%
...
saveRDS(all_pcs, file = "Petiolaris.tranche90.snp.petpet.90.bi.lostruct.1000.all_pcs.rds")
...
pdf("Petiolaris.tranche90.snp.petpet.90.bi.lostruct.1000.closeup.v0.pdf")


all_pcs %>%
  filter(chrom == "HanXRQChr04") %>%
  select(chrom, start, end, mid, mds1, mds3) %>%
  distinct(chrom, start, end, mid,mds1, mds3) %>% 
  gather(., mds, value,  mds1, mds3) %>%
  ggplot(.,aes(x=mid/1000000,y=value)) + geom_point() +
  theme_bw() + xlab("MB") + ylab("MDS") +
  ggtitle("MDS 4a") +
  facet_wrap(~mds,nrow=3,scale="free")


all_pcs %>%
  filter(chrom == "HanXRQChr05") %>%
  select(chrom, start, end, mid, mds1, mds3, mds4) %>%
  distinct(chrom, start, mid, end, mid, mds1, mds3, mds4) %>% 
  gather(., mds, value, mds1, mds3, mds4) %>%
  ggplot(.,aes(x=mid/1000000,y=value)) + geom_point() +
  theme_bw() + xlab("MB") + ylab("MDS") +
  ggtitle("MDS 5b") +
  facet_wrap(~mds,nrow=5,scale="free")


all_pcs %>%
  filter(chrom == "HanXRQChr06") %>%
  select(chrom, start, end, mid, mds5, mds6, mds7) %>%
  distinct(chrom, start, mid, end, mid, mds5, mds6, mds7) %>% 
  gather(., mds, value, mds5, mds6, mds7) %>%
  ggplot(.,aes(x=mid/1000000,y=value)) + geom_point() +
  theme_bw() + xlab("MB") + ylab("MDS") +
  ggtitle("MDS 6b") +
  facet_wrap(~mds,nrow=5,scale="free")
...
all_pcs %>%
  filter(chrom == "HanXRQChr07") %>%
  select(chrom, start, end, mid, mds1, mds3 ) %>%
  distinct(chrom, start, end, mid, mds1, mds3) %>% 
  gather(., mds, value, mds1, mds1, mds3) %>%
  ggplot(.,aes(x=mid/1000000,y=value)) + geom_point() +
  theme_bw() + xlab("MB") + ylab("MDS") +
  ggtitle("MDS 7a") +
  facet_wrap(~mds,nrow=5,scale="free")
...
all_pcs %>%
  ...
  select(chrom, start, end, mid,mds2, mds3, mds4 ) %>%
  distinct(chrom, start, end, mid,mds2, mds3, mds4) %>% 
  gather(., mds, value, mds2, mds3, mds4) %>%
  ggplot(.,aes(x=mid/1000000,y=value)) + geom_point() +
  theme_bw() + xlab("MB") + ylab("MDS") +
  ggtitle("MDS 9a") +
  facet_wrap(~mds,nrow=5,scale="free")
...
all_pcs %>%
  ...
  select(chrom, start, end, mid, mds2,mds3 ) %>%
  distinct(chrom, start, end, mid, mds2,mds3) %>% 
  gather(., mds, value, mds2,mds3) %>% 
  ggplot(.,aes(x=mid/1000000,y=value)) + geom_point() +
  theme_bw() + xlab("MB") + ylab("MDS") +
  ggtitle("MDS 11b") +
  facet_wrap(~mds,nrow=5,scale="free")
...
all_pcs %>%
  ...
  select(chrom, start, end, mid, mds3,mds5,mds6 ) %>%
  distinct(chrom, start, end, mid, mds3,mds5,mds6) %>% 
  gather(., mds, value, mds3,mds5,mds6) %>% 
  ggplot(.,aes(x=mid/1000000,y=value)) + geom_point() +
  theme_bw() + xlab("MB") + ylab("MDS") +
  ggtitle("MDS 12a") +
  facet_wrap(~mds,nrow=5,scale="free")
...
all_pcs %>%
  ...
  select(chrom, start, end, mid, mds5,mds6, mds8 ) %>%
  distinct(chrom, start, end, mid,mds5,mds6, mds8) %>% 
  gather(., mds, value, mds5,mds6, mds8) %>% 
  ggplot(.,aes(x=mid/1000000,y=value)) + geom_point() +
  theme_bw() + xlab("MB") + ylab("MDS") +
  ggtitle("MDS 14c") +
  facet_wrap(~mds,nrow=5,scale="free")
...
all_pcs %>%
  filter(chrom == "HanXRQChr17") %>%
  select(chrom, start, end, mid, mds3,mds5, mds6, mds7 ) %>%
  distinct(chrom, start, end, mid, mds3,mds5, mds6, mds7) %>% 
  gather(., mds, value, mds3,mds5, mds6, mds7) %>% 
  ggplot(.,aes(x=mid/1000000,y=value)) + geom_point() +
  theme_bw() + xlab("MB") + ylab("MDS") +
  ggtitle("MDS 17ab") +
  facet_wrap(~mds,nrow=5,scale="free")
...
inversions = c("4a","5b", "6b", "7a", "9a","11b","12a","14c", "17a","17b")


pdf("Petiolaris.tranche90.snp.petpet.90.bi.inversions.pdf")
all_calls <- tibble(name=character(),haplotype=numeric(),region=character())
for (inversion in inversions){
  struc = read_delim(paste("/media/owens/Copper/wild_gwas_2018/petpet/Petiolaris.tranche90.snp.petpet.90.bi.",inversion,".2.meanQ",sep=""),col_names = F,
                     delim=" ")
  
  het = read_tsv(paste("/media/owens/Copper/wild_gwas_2018/petpet/Petiolaris.tranche90.snp.petpet.90.bi.",inversion,".het.txt",sep=""))
  colnames(struc) <- c("k1","nothing","k2")
  struc %>% select(-nothing) -> struc
  cbind(struc, samples) %>% rename(name = sample)-> struc
  
  
  folder <- "/media/owens/Copper/wild_gwas_2018/petpet/"
  gds.file <- paste("Petiolaris.tranche90.snp.petpet.90.bi.",inversion,".gds",sep="")
  vcf.file <- paste("Petiolaris.tranche90.snp.petpet.90.bi.",inversion,".vcf",sep="")
  ...
  print(
    if (nrow(tab %>% inner_join(., struc) %>%filter(k1 < 0.75, k1 > 0.25)) > 0){
      ...
      ggplot(usa, aes(long, lat)) +
        ...
        geom_scatterpie(data=tab %>%
                          ...
                          spread(., haplotype, n,fill=0),
                        aes(x=long, y=lat, r=1.0), 
                        cols=c("0","2"), color=NA, alpha=.8) +
        scale_fill_manual(name="Haplotype",values=brewer.pal(3,"Set1")[c(1,3)]) +theme_bw() +
        xlab("Longitude") + ylab("Latitude") + 
        ggtitle(paste("Region",inversion)) +
        coord_map(xlim = c(-115,-96),ylim = c(30, 42))
    }
...
pdf("Petiolaris.tranche90.snp.petpet.90.bi.inversions.genotyped.pdf")
...
all_calls %>% 
  inner_join(labels) %>%
  write_tsv(.,"Petiolaris.tranche90.snp.petpet.90.bi.inversions.genotyped.txt")


read_tsv("Petiolaris.tranche90.snp.petpet.90.bi.inversions.genotyped.txt") %>%
  mutate(color = case_when(haplotype == 0 ~ brewer.pal(6,"Set1")[1],
                           haplotype == 1 ~ brewer.pal(6,"Set1")[2],
                           haplotype == 2 ~ brewer.pal(6,"Set1")[3])) %>%
  select(name,region,color) %>%
  write_tsv(., "Petiolaris.tranche90.snp.petpet.90.bi.inversions.phylocolors.txt")

####Plotting fst across inversions
inversions = c("4a","5b", "6b", "7a", "9a","11b","12a","14c", "17a","17b")

pdf("Petiolaris.tranche90.snp.petpet.90.bi.inversions.genotyped.fst.pdf",height=17,width=20)
for (inversion in inversions){
  
  fst <- read_tsv(paste("/media/owens/Copper/wild_gwas_2018/petpet/Petiolaris.tranche90.snp.petpet.90.bi.",inversion,".fst.txt",sep=""))
