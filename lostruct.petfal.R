...
data_name <- "Petiolaris.tranche90.snp.petfal.90.bi"
...
system.time( snp.pca <- eigen_windows(win.fn.snp,k=2, mc.cores=9) )
...
#Special pulling out window on HanXRQChr09, which is on mds02_neg, but correlated to other blocks
win.regions %>%
  mutate_(the_mds = "mds02" ) %>%
  summarize(sd_mds = sd(the_mds)) %>% pull() -> sd_mds
mds_high_cutoff <- sd_mds * 3
mds_low_cutoff <- sd_mds * 2.5
tmp <- tibble(mds_coord = as.character("mds02"),direction = as.character("neg"),clust_pvalue = as.numeric(NA), outliers=as.numeric(NA),
              n1_outliers=as.numeric(NA), high_cutoff = as.numeric(-mds_high_cutoff),lower_cutoff=as.numeric(-mds_low_cutoff),
              chr=as.character("HanXRQChr09"))
sig_mds_clusters <- rbind(tmp,sig_mds_clusters)
...
for (i in 1:nrow(sig_mds_clusters)){
  ...
  if (direction == "pos"){
    current_windows <- win.regions %>%
     ...
     dplyr::(-ahead_n, -behind_n, -min_dist)
...
for (i in 1:nrow(sig_mds_clusters)){
  ...
  if (direction == "pos"){
    current_windows <- win.regions %>%
      ...
      mutate(min_dist = pmin(ahead_n, behind_n,na.rm=T)) %>%
      filter(min_dist < max_distance_between_outliers ) %>%
      dplyr::select(-ahead_n, -behind_n, -min_dist)
